...
function iti_widget_image_return(iti_widget_image_id, iti_widget_image_thumb) {
  ...
  iti_widget_image_context.append(
    '<img src="' + iti_widget_image_thumb + '" alt="Image" />'
  );
...
function iti_widget_image_update_thickbox() {
  if (iti_widget_image_context) {
      ...
      jQuery("#TB_iframeContent")
        ...
        .each(function () {
          if (jQuery(this).find("input.iti-widget-image-choose").length == 0) {
            ...
            jQuery(this).prepend(
              '<input type="submit" name="itiwidgetimagechoose" class="iti-widget-image-choose button" value="Use this image" />'
            );
          }
        });
    }
    ...
    jQuery("#TB_iframeContent")
      .contents()
      .find("td.savesend input.iti-widget-image-choose")
      .unbind("click")
      .click(function (e) {
        e.preventDefault();
        iti_widget_image_parent = jQuery(this).parent().parent().parent();
        iti_widget_image_id = iti_widget_image_parent
          .find("td.imgedit-response")
          .attr("id")
          .replace("imgedit-response-", "");
        iti_widget_image_thumb = iti_widget_image_parent
          .parent()
          .parent()
          .find("img.pinkynail")
          .attr("src");
    ...
    if (
      jQuery("#TB_iframeContent")
        .contents()
        .find(".media-item .savesend input[type=submit], #insertonlybutton")
        .length
    ) {
      jQuery("#TB_iframeContent")
        .contents()
    ...
    if (jQuery("#TB_iframeContent").contents().find("tr.post_title").length) {
      ...
      jQuery("#TB_iframeContent")
        .contents()
        .find('tr.image-size input[value="full"]')
        .prop("checked", true);
      jQuery("#TB_iframeContent")
        .contents()
        .find(
          "tr.post_title,tr.image_alt,tr.post_excerpt,tr.image-size,tr.post_content,tr.url,tr.align,tr.submit>td>a.del-link"
        )
        .hide();
    }
  }
  ...
  if (
    jQuery("#TB_iframeContent").contents().length == 0 &&
    iti_widget_image_context
  ) {
...
jQuery(document).ready(function () {
  jQuery(".widgets-holder-wrap").on(
    "click",
    "a.iti-image-widget-trigger",
    function (e) {
      e.preventDefault();
      ...
      jQuery(this).attr(
        "href",
        href + "&width=" + (W - 80) + "&height=" + (H - 85)
      );
      iti_widget_image_context = jQuery(this)
        .parent()
        .find(".iti-image-widget-image");
      jQuery("#TB_title").remove(); // TODO: why is this necessary?
      tb_show(jQuery(this).attr("title"), event.target.href, false);
      iti_widget_image_thickbox_updater = setInterval(
        iti_widget_image_update_thickbox,
        500
      );
    }
  );
